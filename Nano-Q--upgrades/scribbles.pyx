...
def doSomeSubprocess(filePath, numReads, toolpath, jump = 2, threadsInt = 3):
    ...
    # For each set of jumps
        ...
        procs.append(proc)    
        intUsedThreads = intUsedThreads + 1;  # account for new launched thread

        while intUsedThreads > threadsInt:
        # while at my max thread limit
            for proc in procs:
            # For all launched process
                if proc.poll() is not None:
                # If a process finshed
                    intUsedThreads = intUsedThreads - 1;     # adjust thread count
                    procs.remove(proc);         # remove the process
                # If a process finshed
            # For all launched process
...
def getSomeHam(str s1, str s2):
    cdef int hamDistInt = 0;
    cdef int intCnt = 0;

    while intCnt < len(s1):
    # while theire are bases go compare
        if s1[intCnt] != s2[intCnt]:
            hamDistInt += 1;
        intCnt = intCnt + 1;
    # while theire are bases go compare
