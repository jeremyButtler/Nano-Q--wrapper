...
StartCodon = 0
Jump = None
threadsInt=3       # number of threads to use
...
if __name__ == '__main__':   
    ...
    *****
    ...
    parser.add_argument('-t','--threads', help="Number of threads to use", action='store_true')
    ...
    if(args.threads):
        threadsInt = args.threads;   # if user set number of threads to use
    # if(args.shift_start):
    ...
    for i in range (0,len(numCleanedList)):
        ...
        if(numCleanedList[i] > 0):
            ...
            if(Jump):
                sc.doSomeSubprocess(fileList[i],numCleanedList[i], toolPath, jump = Jump, threadsInt = threadsInt)
            else:
                sc.doSomeSubprocess(fileList[i],numCleanedList[i], toolPath, threadsInt = threadsInt)
        else:            
